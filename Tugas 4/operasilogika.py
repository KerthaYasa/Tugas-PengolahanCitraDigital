...
import numpy as np  # hanya untuk konversi list ke array agar bisa ditampilkan
...
def binarize(img, h, w, threshold=0.75, invert=False):
    ...
    for y in range(h):
        ...
        for x in range(w):
            g = to_gray(img[y][x])
            bit = 1 if g >= threshold else 0  # objek putih = 1
            if invert:
                bit = 1 - bit
            row.append(bit)
        hasil.append(row)
...
def logic_not(a): return 1 - a

hasil_and, hasil_or, hasil_xor, hasil_sub, hasil_not = [], [], [], [], []

for y in range(height):
    row_and, row_or, row_xor, row_sub, row_not = [], [], [], [], []
    for x in range(width):
        ...
        row_not.append([logic_not(a)] * 3)
    hasil_and.append(row_and)
    hasil_or.append(row_or)
    hasil_xor.append(row_xor)
    hasil_sub.append(row_sub)
    hasil_not.append(row_not)

# === KONVERSI KE ARRAY FLOAT ===
hasil_and = np.array(hasil_and, dtype=float)
hasil_or = np.array(hasil_or, dtype=float)
hasil_xor = np.array(hasil_xor, dtype=float)
hasil_sub = np.array(hasil_sub, dtype=float)
hasil_not = np.array(hasil_not, dtype=float)
...
axes[6].imshow(hasil_not, cmap='gray', vmin=0, vmax=1)
axes[6].set_title('NOT A')

# Kosongkan satu slot terakhir biar layout tetap 2x4 rapi
axes[7].axis('off')
